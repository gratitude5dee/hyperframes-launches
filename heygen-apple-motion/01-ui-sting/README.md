...
a sign-in card, a counter, a nav, a click, a menu, an endcard — cut together so it
reads as one continuous camera move rather than seven scenes in a row.
...
*"turn this into a video for Figma."* Everything the agent needs is in the next section —
...
| # | frames | what happens |
|---|---|---|
| A | 1–35 | Lockup fades up and grows. Wordmark and mark **counter-bounce** in antiphase, then the whole thing rises out of frame, accelerating. |
...
| D | 114–154 | Counter card rises in, the number runs 0 → 5,278 on an odometer, exits right. |
...
- **Colour** — `#00c3ff` accent, `#10121a` ink, `#f2f4f7` ground, `#d5d9e0` hairlines.
...
## Fonts — read before publishing

`assets/fonts/` contains **TT Norms Pro** and **ABC Solar Display**. These are
commercially licensed faces, not open ones. A licence to use them in your own work
does not automatically cover redistributing the files in a public repository.

Confirm your licence covers it, or delete `assets/fonts/` and the two `@font-face`
blocks at the top of `index.html` — the stack falls back to `sans-serif` cleanly, and
the layout still holds because no width is measured at runtime.

The faces are registered under private family names (`HGSans`, `HGDisplay`) rather
than their real ones. That is deliberate: the renderer silently aliases system font
names, and you get a substituted face with no error.
