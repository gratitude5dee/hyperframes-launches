...
a log-in card, a follower counter, a nav, a click, a create menu, an endcard — cut
...
*"turn this into a video for Notion."* Everything the agent needs is in the next section —
...
"turn this into a video for Notion."** That is the whole brief you are going to get.
...
| D | 114–154 | Follower card rises in, the number runs 0 → 5,278 on an odometer, exits right. |
...
| F | 210–304 | The outlined **+** button arrives oversized and contracts, its plus rotating a quarter turn as it lands. A cursor walks in and clicks; the click opens the menu; the group nudges aside to give the menu the frame. |
...
- **Colour** — `#0095f6` accent, `#262626` ink, `#fafafa` ground, `#dbdbdb` hairlines.
- **The create affordance** — this example uses an icon-only outlined squircle rather
  than a labelled pill, so three numbers move together: the button's own box, the
  menu's `left`/`top` (it has to open off the button's corner instead of over it), and
  the group nudge that re-centres the menu afterwards. Change one, change all three.
- **Entry scale** — `#f-btn` enters from `scale: 3` rather than the ~1.56 a wide pill
  uses. A small icon needs a bigger multiple or the cut from the full-frame nav reads as
  a drop in mass. Raising it trades velocity match at that seam for presence; the
  direction of travel (both sides shrinking) is what actually has to hold.
...
`assets/fonts/` ships **Inter** (SIL OFL). It is an open licence, so this example
...
demo, not an official Instagram asset, and it is not affiliated with or endorsed by Instagram.
Swap `assets/instagram-mark.svg` and the strings in `index.html` for your own brand before shipping
anything real.
